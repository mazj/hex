...
static
int _hex_semantics_check_and_get_identifier(Expr expr, char **identifier, Semantics semantics)
{
  HEX_ASSERT(expr);
  ...
  *identifier = primary_expr->primary_expr_identifier;
  ...
  Declaration declaration, unsigned int indent_level, Semantics semantics)
{
  ...
  while(_exprlist->next) {
    ...
    HEX_SEMANTICS_CHECK_AND_RETURN(
      _hex_semantics_check_and_get_identifier(
        _expr,
        &identifier_name,
        semantics
      )
...
{
  HEX_ASSERT(index_expr);
  HEX_ASSERT(semantics);

  switch(index_expr->postfix_index_expr_type)
  {
